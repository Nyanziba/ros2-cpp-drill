...
**`.vscode/` を同梱しているので、リポジトリのフォルダを開いていれば出ないはずです。**
サブフォルダだけを開いていると効かないので、リポジトリの直下を開いてください。

`/opt/ros/jazzy` がホストに無い（Ubuntu 以外）場合は、そもそもヘッダがありません。
コマンドパレットから **「Dev Containers: Reopen in Container」** を実行すると、
コンテナの中で開けるので解決します。
